...
t_slds_SliderArrowsHeight(recid);t_slds_ActiveSlide(recid,pos,totalSlides);t_slds_initSliderControls(recid);t_slds_ActiveCaption(recid,pos);if(sliderWrapper.attr('data-slider-timeout')>0){t_slds_initAutoPlay(recid)}
...
sliderWrapper.addClass('t-slds_animated');sliderWrapper.css({transform:'translate3d(-'+(sliderWidth*pos)+'px, 0, 0)'});setTimeout(function(){sliderWrapper.removeClass('t-slds_animated');sliderWrapper.attr('data-slider-animated','');if(cycle=='yes'){if(pos==(totalSlides+1)){pos=1};if(pos==0){pos=totalSlides};sliderWrapper.css({transform:'translate3d(-'+(sliderWidth*pos)+'px, 0, 0)'});sliderWrapper.attr('data-slider-pos',pos);t_slds_ActiveBullet(recid,pos);t_slds_ActiveSlide(recid,pos,totalSlides);t_slds_ActiveCaption(recid,pos);if(el.find('.t-slds__item-loaded').length<totalSlides+2){t_slds_UpdateImages(recid,pos);if(window.lazy=='y'){t_lazyload_update()}}}
if(el.find('.t-slds__item-loaded').length<totalSlides+2){if(window.lazy=='y'){t_lazyload_update()}}},sliderTransition);t_slds_ActiveBullet(recid,pos);t_slds_ActiveSlide(recid,pos,totalSlides);if(sliderWrapper.attr('data-slider-correct-height')=='true'){t_slds_SliderHeight(recid)}
t_slds_SliderArrowsHeight(recid);t_slds_ActiveCaption(recid,pos);if(el.find('.t-slds__item-loaded').length<totalSlides+2){t_slds_UpdateImages(recid,pos)}}
function t_slds_UpdateImages(recid,pos){var el=$('#rec'+recid),item=el.find('.t-slds__item[data-slide-index="'+pos+'"]');item.addClass('t-slds__item-loaded');item.next().addClass('t-slds__item-loaded');item.prev().addClass('t-slds__item-loaded');el.trigger('updateSlider')}
function t_slds_ActiveCaption(recid,pos){var el=$('#rec'+recid),caption=el.find('.t-slds__caption'),captionActive=el.find('.t-slds__caption[data-slide-caption="'+pos+'"]');caption.fadeOut(0);captionActive.fadeIn(300)}
...
function t_slds_ActiveBullet(recid,pos){var el=$('#rec'+recid),bullet=el.find('.t-slds__bullet'),bulletActive=el.find('.t-slds__bullet[data-slide-bullet-for="'+pos+'"]');bullet.removeClass('t-slds__bullet_active');bulletActive.addClass('t-slds__bullet_active')}
