...
if ($GLOBAL["sitekey"] == 1 && $GLOBAL["database"] == 1) {
    ...
    if (!is_file("banners-sets.dat")) {
        $AdminText = "Не найден файл &laquo;<b>/admin/banners-sets.dat</b>&raquo; создайте его вручную и поставьте права 0777";
        $GLOBAL["error"] = 1;
    } else {
        $sets = explode("|", @file_get_contents("banners-sets.dat"));
        $table3 = $sets[0]."_items";
...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
        ...
        if (isset($P["savebutton"])) {
            ...
            $data = DB(
              "SELECT * FROM `_banners_orders` WHERE (`zid`='".$P["zay"]."')"
            );
            @mysql_data_seek($data["result"], 0);
            $ar = @mysql_fetch_array($data["result"]);
            if ($data["total"] != 1) {
                $error = 1;
                $_SESSION["Msg"] = "<div class='ErrorDiv'>Заявка не найдена на сервере $P[zay]</div>";
            }

            if ($P["deluserflash"] == "1") {
                @unlink($ROOT."/advert/files/flash/".$P["oflash"]);
                $P["oflash"] = '';
            }
            if (isset($F["userflash"]) && $F["userflash"]["tmp_name"] != "") {
                @require_once($ROOT."/admin/modules/banners/UPL-flash.php");
            } else {
                $vflash = $P["oflash"];
            }
            if ($P["deluserpic"] == "1") {
                @unlink($ROOT."/advert/files/image/".$P["opic"]);
                $P["opic"] = '';
            }
            if (isset($F["userpic"]) && $F["userpic"]["tmp_name"] != "") {
                @require_once($ROOT."/admin/modules/banners/UPL-image.php");
            } else {
                $vpic = $P["opic"];
            }
            if ($P["delusermobile"] == "1") {
                @unlink($ROOT."/advert/files/mobile/".$P["omobile"]);
                $P["omobile"] = '';
            }
            if (isset($F["usermobile"]) && $F["usermobile"]["tmp_name"] != "") {
                @require_once($ROOT."/admin/modules/banners/UPL-mobile.php");
            } else {
                $vmobile = $P["omobile"];
            }
            
            if ($P["deluserhtml5"] == "1") {
                @unlink($ROOT."/advert/files/html5/".$P["ohtml5"]);
                $P["ohtml5"] = '';
            }
            if (isset($F['userhtml5']) && $F['userhtml5']['tmp_name'] !== '') {
                require_once $ROOT.'/admin/modules/banners/UPL-html5.php';
            } else {
                $html5 = $P['ohtml5'];
            }
			...
			WHERE (`id`='".(int)$id."');";
                ...
                if ($_SESSION["Msg"] == "" || !isset($_SESSION["Msg"])) {
                    $_SESSION["Msg"] = "<div class='SuccessDiv'>Материал успешно сохранен! <a target='_blank' href='/advert/preBanner.php?id=$id'><b>Просмотр</b></a></div>";
                }
            }
            @header("location: ?cat=banners_edit&id=".$id);
            exit();
        }
...
// ВЫВОД ПОЛЕЙ И ФОРМ
        ...
        $data = DB(
          "SELECT * FROM `_banners_items` WHERE (`id`='".(int)$id."')"
        );
        @mysql_data_seek($data["result"], 0);
        $n = @mysql_fetch_array($data["result"]);
	...
	<tr class="TRLine0"><td class="VarText">Начало показов</td><td class="DateInput">'.GetDataSet(
            $n["datafrom"]
          ).'</td></tr>	<tr class="TRLine0"><td class="Vartext">Конец показов</td><td class="DateInput">'.GetDataSet(
            $n["datato"],
            1
          ).'</td></tr>
	...
	<tr class="TRLine1"><td class="Vartext">HTML5</td><td><input type="file" name="userhtml5" style="border:2px solid #FFF; border-radius:5px;"><div style="float:right; padding:5px 0 0 0;">удалить: <input type="checkbox" name="deluserhtml5" value="1"></div></td></tr>
	...
	<input type="hidden" name="ohtml5" value="' . $n['html5'] . '"><input type="hidden" name="oflash" value="'.$n["flash"].'" /><input type="hidden" name="opic" value="'.$n["pic"].'" /><input type="hidden" name="omobile" value="'.$n["mobile"].'" />'."</table><script>LoadZayavka();</script></div>";
...
// ПРАВАЯ КОЛОНКА
        ...
        $AdminRight .= "<h2 id='zcap'>Заявка</h2><div class='RoundText' id='Info'>Укажите номер заявки</div>".$C5.
        ...
        if ($n['flash'] != "") {
            $AdminRight .= "<b>Flash</b>: <a target='_blank' href='/advert/files/flash/$n[flash]'>$n[flash]</a>";
        } else {
            $AdminRight .= "<b>Flash</b>: НЕТ";
        }
        $AdminRight .= "<hr>";
        if ($n['pic'] != "") {
            $AdminRight .= "<b>Фото</b>: <a target='_blank' href='/advert/files/image/$n[pic]'>$n[pic]</a>";
        } else {
            $AdminRight .= "<b>Фото</b>: НЕТ";
        }
        $AdminRight .= "<hr>";
        if ($n['mobile'] != "") {
            $AdminRight .= "<b>Моб.</b>: <a target='_blank' href='/advert/files/mobile/$n[mobile]'>$n[mobile]</a>";
        } else {
            $AdminRight .= "<b>Моб.</b>: НЕТ";
        }
        $AdminRight .= '<hr>';
        if ($n['html5'] !== '') {
            $AdminRight .= '<b>HTML5</b>: <a href="/advert/files/html5/'.$n['html5'].'">'.$n['html5'].'</a>';
        } else {
            $AdminRight .= '<b>HTML5</b>: НЕТ';
        }
