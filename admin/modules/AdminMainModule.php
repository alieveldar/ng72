...
function GetOnlyDataSet($data=0, $add="") {
	global $GLOBAL; if ($data==0) { $data=mktime()+($GLOBAL["timezone"]*3600); } $h=date("H", $data); $i=date("i", $data); $s=date("s", $data);
	$text='<input id="datepick'.$add.'" name="ddata1'.$add.'" type="text" readonly value="'.date("d.m.Y", $data).'">'; return $text;
}
...
function translit($s) {
  ...
  $s = preg_replace("/-+/", '-', $s); // удаляем повторяющие минусы
