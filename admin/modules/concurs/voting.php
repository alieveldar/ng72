...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if ($node["winnerscount"]==1) { $vw1="selected"; } elseif ($node["winnerscount"]==2) { $vw2="selected"; } elseif ($node["winnerscount"]==13) { $vw13="selected"; } else { $vw3="selected"; }	
	...
	$AdminText.='<tr class="TRLine0"><td class="VarText">Кол-во победителей</td><td class="LongInput"><div class="sdiv"><select name="winnerscount"><option value="1" '.$vw1.'>1</option><option value="2" '.$vw2.'>2</option><option value="3" '.$vw3.'>3</option><option value="13" '.$vw13.'>13</option></select></div></td><tr>';
	...
	$data=DB("SELECT `vid`, COUNT(`id`) as `cnt` FROM `_widget_votes` WHERE (`pid`='".(int)$id."' && `link`='".$alias."') GROUP BY `vid`"); $vts=array();
	for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"],$i); $ar=@mysql_fetch_array($data["result"]); $vts[$ar["vid"]]=$ar["cnt"]; endfor;
	...
	if ($data["total"]>0) {
		...
		for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"],$i); $ar=@mysql_fetch_array($data["result"]); if ($ar["stat"]==1) { $chk0="checked"; }
			...
			<td class="LongInput" style="width:10%;" valign="top" align="center">'.
			"<a style='color:red;' href='?cat=".$alias."_status&id=".$id."&vid=".$ar["id"]."' target='_blank'>Статус голосов</a>: <b>".$vts[$ar["id"]]."</b>"
			.$C10.$img.$C10.'<input type="checkbox" id="RS-'.$ar["id"].'-_widget_pics" value="1" '.$chk0.' /></td>
