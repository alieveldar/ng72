...
if ($GLOBAL["sitekey"]==1 && $GLOBAL["database"]==1) {
	...
	$q1="SELECT ".$table1.".*, ".$table.".`name` as `rname`, `ctable`.`name` as `cname`, ".$table2.".`login`, ".$table2.".`name` as `uname`, ".$table2.".`phone` as `uphone`, ".$table3.".`id` as `payid`, ".$table3.".`price` as `tprice`, ".$table3.".`stat` as `paystat` 
	FROM ".$table1."
	LEFT JOIN ".$table3." ON ".$table3.".`oid`=".$table1.".`id`
	LEFT JOIN ".$table2." ON ".$table2.".`id`=".$table1.".`uid`
	LEFT JOIN ".$table." ON ".$table.".`id`=".$table1.".`rid`
	LEFT JOIN ".$table." as `ctable` ON `ctable`.`id`=".$table.".`pid`  
	...
	if ($ar["paystat"]==1) { $stat="<B style='font-size:10px; color:green;'>ОПЛАЧЕНО</B>"; } else { $stat="<B style='font-size:9px; color:red;'>ждет оплаты</B>"; }
	if ($a[0]==1){ $d1="<B style='color:green;'>ДА</B>"; } else { $d1="нет"; } if ($a[1]==1){ $d2="<B style='color:green;'>ДА</B>"; } else { $d2="нет"; } if ($a[2]==1){ $d3="<B style='color:green;'>ДА</B>"; } else { $d3="нет"; }
	if ($a[3]==1){ $d4="<B style='color:green;'>ДА</B>"; } else { $d4="нет"; } if ($a[4]==1){ $d5="<B style='color:green;'>ДА</B>"; } else { $d5="нет"; } $ds=explode(",", $ar["datas"]); 
	
$obj=str_replace(array("\r\n", "\r", "\n", "'"), "", "<h2 style='margin:0; padding:10px 0 0 0;'>Пользователь ID=".$ar["uid"]."</h2>Имя: <b>".$ar["uname"]."</b><!--  ,  Логин: <b>".$ar["login"]."</b>-->  ,  Телефон: <b>".$ar["uphone"]."</b><div class='C20' style='width:700px;'></div><h2 style='margin:0; padding:10px 0 0 0;'>Объявление ID=".$ar["id"]." , Счет на оплату ID=".$ar["payid"]."</h2>Сумма: ".$price."<br>Статус: <b>".$stat2."</b><br>Создано: <b>".$datan[1]."</b><!--<br>Выходов: <b>".count($ds)."</b>--><br>Текст: <b>".str_replace('"', "`", $ar["text"])."</b><br>Контакт: <b>".$ar["phone"]."</b><br>Раздел: <b>".$ar["cname"]."</b> / <b>".$ar["rname"]."</b><br>Выходы: <b>".$ar["datas"]."</b><br><div class='C20' style='width:700px;'></div><h2 style='margin:0; padding:10px 0 0 0;'>Дополнительно:</h2>Синий фон + рамка + крупный номер: <b>".$d1."</b><br>Выделено рамкой: <b>".$d2."</b><br>Выделено жирным шрифтом: <b>".$d3."</b><br>Выделено обводкой: <b>".$d4."</b><br>Цвет + выделено жирным шрифтом: <b>".$d5."</b><br><div class='C20' style='width:700px;'></div><h2 style='margin:0; padding:10px 0 0 0;'>Админ инфо:</h2>".$ar["dop"]);
