...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
        ...
        if (isset($P["addbutton"])) {
            if (isset($_FILES["photo"]["name"]) && $_FILES["photo"]["name"] != "") {
                ...
                if ($loaded == 1) {
                    $class = "SuccessDiv";
                    $q = "UPDATE `" . $alias . "_specprj` SET `pic`='" . $picname . "', `picxy`='" . $picxy . "' WHERE (id='" . (int)$id . "')";
                    DB($q);
                } else {
                    $class = "ErrorDiv";
                }
                DB("INSERT INTO `_specprjlog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('" . $alias . "', '" . $id . "', '" . $_SESSION['userid'] . "', '" . time() . "', '" . $_SERVER['REMOTE_ADDR'] . "', 'Загрузка фотографии (photo): /userfiles/picoriginal/" . $picname . "')");
            }
        ...
        if (isset($P["savebutton"])) {
            ...
            $q = "UPDATE `" . $alias . "_specprj` SET `picxy`='" . $picxy . "', `picauth`='" . $P["auth"] . "' WHERE (id='" . (int)$id . "')";
            DB($q);
            DB("INSERT INTO `_specprjlog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('" . $alias . "', '" . $id . "', '" . $_SESSION['userid'] . "', '" . time() . "', '" . $_SERVER['REMOTE_ADDR'] . "', 'Сохранение фотографии (photo): " . $P["auth"] . "');");
        ...
        // ВЫВОД ПОЛЕЙ И ФОРМ
        $data = DB("SELECT `pic`, `picxy`, `name`, `picauth`, `stat` FROM `" . $alias . "_specprj` WHERE (`id`='" . (int)$id . "') LIMIT 1");
        if ($data["total"] != 1) {
            ...
            $OriginalPic = "userfiles/picoriginal/" . $node["pic"];
            $PreViewPic = "userfiles/picitem/" . $node["pic"] . "?r=" . time();
            ...
            $AdminText .= $C5 . "<div class='Info'><input type='hidden' name='exclusive' value='1'>  </div>";
            $AdminText .= "</div></form>";
	...
	<div class='SecondMenu2'><a href='/$alias/view/$id/' target='_blank'>Просмотр на сайте</a></div></form>";
            ...
            if ($node["pic"] != "") {
                $AdminRight .= $C10 . $C10 . $C10 . "<h2>Пример превью:</h2>" . $C5;
                $AdminRight .= "<img src='/" . $PreViewPic . "' style='width:220px;'>";
            }
