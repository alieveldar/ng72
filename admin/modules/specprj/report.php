...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		foreach ($P["Inp"] as $key=>$val) {
			$ch=(int)$P["Inb"]["$key"]; $name=$val; $text=$P["PostText"][$key]; 
			$q="UPDATE `_widget_pics` SET `name`='".$name."', `text`='".$text."', `sets`='".$ch."' WHERE (`id`='".(int)$key."' && pid='".(int)$id."')"; DB($q);
		}
		DB("INSERT INTO `_specprjlog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('".$alias."', '".$id."', '".$_SESSION['userid']."', '".time()."', '".$_SERVER['REMOTE_ADDR']."', 'Редактирование фотоотчета (report)')");
		$_SESSION["Msg"]="<div class='SuccessDiv'>Настройки успешно сохранены</div>"; @header("location: ".$_SERVER["REQUEST_URI"]); exit();
	}	
	
	
	// ВЫВОД ПОЛЕЙ И ФОРМ
	$data=DB("SELECT `name`, `stat` FROM `".$alias."_specprj` WHERE (`id`='".(int)$id."') LIMIT 1");
