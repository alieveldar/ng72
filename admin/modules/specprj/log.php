...
// РАЗДЕЛ
    ...
    if ($data["total"] != 1) {
        ...
        if ($data["total"] != 1) {
            ...
            $AdminText = '<h2>Лог: &laquo' . $node["name"] . '&raquo;</h2>';

            $q = "SELECT `_specprjlog`.*, `_users`.`login`, `_users`.`nick`, `_users`.`role` FROM `_specprjlog` LEFT JOIN `_users` ON `_users`.`id`=`_specprjlog`.`uid` WHERE (`_specprjlog`.`id`='" . (int)$id . "' && `_specprjlog`.`link`='" . $alias . "') ORDER BY `_specprjlog`.`data` ASC";
            $data = DB($q);
            if ($data["total"] == 0) {
                $text = "Лог для этой публикации недоступен.";
            } else {
                $text = "";
            }
	...
	<div class='SecondMenu2'><a href='/$alias/view/$id/' target='_blank'>Просмотр на сайте</a></div></form>";
            if ($_SESSION['userrole'] > 2) {
                $AdminRight .= "<br><div class='SecondMenu2'><a href='?cat=" . $alias . "_log&id=" . $id . "'>Лог редактирования записи</a></div>";
            }
