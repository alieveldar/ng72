...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
        ...
        if (isset($P["savebutton"])) {
            ...
            $aliasid = translit(str_replace(".", "-", $P["ddata1"]) . "-" . str_replace("'", "\'", $P["dtitle"]));

            if (!isset($P['preview_mobile'])) {
                $P['preview_mobile'] = 0;
            }

            $vk_np = DB("SHOW COLUMNS FROM {$alias}_specprj WHERE `Field` = 'vk_np'");
            if (0 == $vk_np['total']) {
                DB("ALTER TABLE {$alias}_specprj ADD vk_np TINYINT(1) NOT NULL DEFAULT 0");
            }

            $q = "UPDATE `" . $alias . "_specprj` SET
		`uid`='" . (int)$P['authid'] . "',
		`bid`='" . (int)$P['bid'] . "',
		`name`='" . str_replace("'", "\'", $P["dname"]) . "',
		`title`='" . str_replace("'", "\'", $P["dtitle"]) . "',
		`alias`='" . $aliasid . "',
		...
		`vk_np`='" . $P['vk_np'] . "'
		WHERE (id='" . (int)$id . "')";
            DB("INSERT INTO `_specprjlog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('" . $alias . "', '" . $id . "', '" . $_SESSION['userid'] . "', '" . time() . "', '" . $_SERVER['REMOTE_ADDR'] . "', 'Сохранение (name): " . str_replace("'", '&#039;', $P["dname"]) . "')");
        ...
        // ВЫВОД ПОЛЕЙ И ФОРМ
        $data = DB("SELECT * FROM `" . $alias . "_specprj` WHERE (`id`='" . (int)$id . "') LIMIT 1");
        if ($data["total"] != 1) {
            ...
            if ($node["bid"] != 0) {
                $bb = DB("SELECT `_banners_items`.*, `_banners_orders`.`id` as `zidd` FROM `_banners_items` LEFT JOIN `_banners_orders` ON `_banners_items`.`zid`=`_banners_orders`.`zid` WHERE (`_banners_items`.`id`='" . (int)$node["bid"] . "') LIMIT 1");
                if ($bb["total"] != 1) {
                    $bst = "<div style='font-size:10px; color:red; margin-top:3px;'>Не найдено записей в баннерной системе с BID=" . (int)$node["bid"] . "</div>";
                } else {
                    @mysql_data_seek($bb["result"], 0);
                    $br = @mysql_fetch_array($bb["result"]);
                    $dr1 = ToRusData($br["datafrom"]);
                    $dr2 = ToRusData($br["datato"]);
                    $bst = "<div style='font-size:11px; color:#777; margin-top:4px;'>BID = <a href='?cat=banners_edit&id=" . (int)$node["bid"] . "' target='_blank'>" . (int)$node["bid"] . "</a>; ZID = <a href='?cat=banners_editorder&id=" . (int)$br["zidd"] . "' target='_blank'>" . (int)$br["zid"] . "</a>;	DATA = " . $dr1[2] . " - " . $dr2[2] . "</div>";
                }
            }
	...
	<tr class="TRLine1"><td class="VarText">Заголовок материала<star>*</star></td><td class="LongInput"><input name="dname" id="dname" type="text" value=\'' . $node["name"] . '\' maxlength="120"></td><tr>
	<!--<tr class="TRLine0"><td class="VarText">Заголовок Яндекса<star>*</star></td><td class="LongInput"><input name="dtitle" id="dtitle" type="text" value=\'' . $node["title"] . '\' maxlength="255"></td><tr>-->
	...
	<tr class="TRLine1 ShowSets"><td class="VarName">Alias материала</td><td class="LongInput"><div class="sdiv"><a href="/' . $alias . '/view/' . $node["alias"] . '" target="_blank">/' . $alias . '/view/' . $node["alias"] . '</a></td><tr>
	...
	<tr class="TRLine1 ShowSets"><td class="VarName">Ссылка на материал</td><td class="LongInput"><input name="realinfo" type="text" value=\'' . $node["realinfo"] . '\'></td><tr>
...
*/
	...
	<td width='20%'>" . $ar["name"] . "</td>";
            ...
            // ПРАВАЯ КОЛОНКА
            $AdminRight = "<br><br>
	<div class='RoundText'><table><tr class='TRLine'><td class='CheckInput'><input type='checkbox' id='RS-" . $id . "-" . $alias . "_specprj' " . $chk . "></td><td><b>Опубликовано</b></td></tr>
	...
	<div class='SecondMenu'><a href='?cat=" . $alias . "_text&id=" . $id . "'>Основное содержание</a></div>";
            if ($alias == "afisha") {
                $AdminRight .= "<div class='SecondMenu'><a href='?cat=" . $alias . "_afisha&id=" . $id . "'>Виджет: Даты для афиши</a></div>";
            }
            /*
	...
	<div class='SecondMenu2'><a href='/$alias/view/$id/' target='_blank'>Просмотр на сайте</a></div></form>";
            */
            $AdminRight .= "<div class='CenterText'><input type='submit' name='savebutton' id='savebutton' class='SaveButton' value='Сохранить данные'></div><br><br>
	<div class='SecondMenu2'><a href='/$alias/view/$id/' target='_blank'>Просмотр на сайте</a></div></form>";
