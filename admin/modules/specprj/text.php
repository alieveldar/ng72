...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		$q="UPDATE `".$alias."_specprj` SET `lid`='".str_replace("'", "\'", $P["lid"])."',`text`='".str_replace("'", "\'",$P["PostText"])."' WHERE (id='".(int)$id."')"; DB($q);
		DB("INSERT INTO `_specprjlog` (`link`, `id`, `uid`, `data`, `ip`, `text`) VALUES ('".$alias."', '".$id."', '".$_SESSION['userid']."', '".time()."', '".$_SERVER['REMOTE_ADDR']."', 'Редактирование содержания (text)')");
		$_SESSION["Msg"]="<div class='SuccessDiv'>Настройки успешно сохранены</div>"; @header("location: ".$_SERVER["REQUEST_URI"]); exit();
	}	
	
	
	// ВЫВОД ПОЛЕЙ И ФОРМ
	$data=DB("SELECT `lid`, `text`, `name`, `stat` FROM `".$alias."_specprj` WHERE (`id`='".(int)$id."') LIMIT 1");
	if ($data["total"]!=1) { $AdminText=ATextReplace('ItemError', $raz["shortname"]." (".$alias.")", $id); $GLOBAL["error"]=1; } else {
		...
		<div class='LongInput' style='float:left; width:620px;'><textarea name='lid' id='lid' style='height:50px; font-size:11px; padding:4px;'>".$node["lid"]."</textarea></div>
