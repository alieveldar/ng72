...
if ($GLOBAL["sitekey"]==1 && $GLOBAL["database"]==1) {
	global $pg;
	$table=$alias."_specprj";
	$table2=$alias."_cats";
	
	$d1=strtotime($id); $d2=$d1+24*60*60;	
	...
	$data=DB("SELECT `".$table."`.*, `".$table2."`.`name` as catn FROM `".$table."` LEFT JOIN `".$table2."` ON `".$table2."`.`id`=`".$table1."`.`cat` WHERE (`".$table."`.`data`>='".$d1."' && `".$table."`.`data`<'".$d2."') 
	...
	$data=DB("SELECT `id` FROM `".$table."` WHERE (`data`>='".$d1."' && `data`<'".$d2."')");
	$AdminText.= Pager($pg, $onpage, ceil($data["total"]/$onpage));
	
	// ПРАВАЯ КОЛОНКА
	$AdminRight="<div class='C20'></div><h2>Календарь публикаций</h2>";
	$defdata=""; if ($id!="" && isset($id)) { $tmp=explode(".", $id); $dirs=$tmp[0].".".$tmp[1].".".$tmp[2]; $defdata='$("#datepick1").datepicker("setDate", "'.$dirs.'")'; }
	$AdminRight.='<div id="datepick1" style="width:200px;"></div><script type="text/javascript">
	$("#datepick1").datepicker({ onSelect: function(dateText, inst) { document.location="?cat='.$alias.'_dater&id="+dateText; }});
