...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		...
		$q="UPDATE `".$alias."_lenta` SET `pay`='".str_replace("'", "\'", $P["lid"])."', `alttext`='".str_replace("'", "\'", $P["PostText"])."',
		`endtext`='".str_replace("'", "\'", $P["Post3Text"])."', `adv`='".str_replace("'", "\'", $P["Post2Text"])."' WHERE (id='".(int)$id."')"; DB($q);
	...
	if ($data["total"]!=1) { $AdminText=ATextReplace('ItemError', $raz["shortname"]." (".$alias.")", $id); $GLOBAL["error"]=1; } else {
		...
		$AdminText.=$C10."<h2>Заключительный текст публикации</h2>
		<textarea name='Post3Text' id='text3edit' style='outline:none;'>".$node["endtext"]."</textarea>";
		$AdminText.=$C10."<h2>Альтернативный текст</h2>
		<textarea name='PostText' id='textedit' style='outline:none;'>".$node["alttext"]."</textarea>";
		$AdminText.=$C10."<h2>Оплаченная часть публикации</h2>
		<textarea name='Post2Text' id='text2edit' style='outline:none;'>".$node["adv"]."</textarea>";
		...
		$AdminText.="<script type='text/javascript'>$(document).ready(function() {
			var beditor=CKEDITOR.replace('textedit'); AjexFileManager.init({ returnTo: 'ckeditor', editor: beditor});
			var beditor2=CKEDITOR.replace('text2edit'); AjexFileManager.init({ returnTo: 'ckeditor', editor: beditor2});
			var beditor3=CKEDITOR.replace('text3edit'); AjexFileManager.init({ returnTo: 'ckeditor', editor: beditor3});
		});</script>";
