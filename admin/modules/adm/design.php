...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["logobutton"])) {
		$userfile=$_FILES['photo']['tmp_name']; $pic="logo.png";
		if (@move_uploaded_file($_FILES['photo']['tmp_name'], $ROOT."/userfiles/temp/".$pic)) {
			copy($ROOT."/userfiles/temp/".$pic, $ROOT."/template/index/".$pic);
			copy($ROOT."/userfiles/temp/".$pic, $ROOT."/template/mobile/".$pic);
			$_SESSION["Msg"]="<div class='SuccessDiv'>Новый логотип успешно загружен!</div>";
		} else { $_SESSION["Msg"]="<div class='ErrorDiv'>Не удалось загрузить логотип!</div>";	}
		@header("location: ".$_SERVER["REQUEST_URI"]); exit();
	}
...
// ВЫВОД ПОЛЕЙ И ФОРМ
	...
	$AdminText.=$C20.'<h2>Логотип сайта (PNG: 150px х 115px)</h2>'.$_SESSION["Msg2"]."<form action='".$_SERVER["REQUEST_URI"]."' enctype='multipart/form-data' method='post'><div class='RoundText'><table><tr><td class='VarName'>Выбрать новый логотип</td><td width='60%'><input name='photo' type='file' accept='.png'></td><td><input type='submit' name='logobutton' id='logobutton' class='SaveButton' value='Загрузить логотип'></td></tr></table></div></form><img src='/template/index/logo.png?time=".time()."'>";
	
// ПРАВАЯ КОЛОНКА	
	$AdminRight=ATextReplace('Design-Module')."<div class='C10'></div>";
}
$_SESSION["Msg"]=""; $_SESSION["Msg2"]="";
?>