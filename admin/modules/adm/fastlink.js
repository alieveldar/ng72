...
function AddNewCross(nid, pid) { var cap="Добавить короткую ссылку"; var text="";
...
text=text+'<tr class="TRLine0"><td>Короткая ссылка<star>*</star></td><td class="NormalInput"><input id="Rn0" type="text" class="JsVerify" placeholder="gazeta"></td></tr>';
text=text+'<tr class="TRLine1"><td>Оригинальная страница<star>*</star></td><td class="NormalInput"><input id="Rl0" type="text" class="JsVerify" value="http://ngzt.ru/" placeholder="http://ngzt.ru/news/view/100"></td></tr>';
text="<div class='RoundText' id='Tgg' style='width:500px;'><table>"+text+"</table><div class='C10'></div>"+button+"</div>"; ViewBlank(cap, text); }

function ItemEdit(name, link, id) { var cap="Редактировать короткую ссылку"; text="";
...
text=text+"<tr class='TRLine0'><td>Короткая ссылка<star>*</star></td><td class='NormalInput'><input id='Rned' type='text' class='JsVerify' value='"+name+"' placeholder='gazeta'></td></tr>";
text=text+"<tr class='TRLine1'><td>Оригинальная страница<star>*</star></td><td class='NormalInput'><input id='Rled' type='text' class='JsVerify' value='"+link+"' placeholder='http://ngzt.ru/news/view/100'></td></tr>";
text="<div class='RoundText' id='Tgg' style='width:500px;'><table>"+text+"</table><div class='C10'></div>"+button+"</div>"; ViewBlank(cap, text); }

function SaveCrossItem(id) { name=$('#Rned').val(); link=$('#Rled').val(); error=0; if (name=="" || name=="NULL"){ error=1; $("#Rned").toggleClass("ErrorInput", true);} if (link=="" || link=="NULL"){ error=1; $("#Rled").toggleClass("ErrorInput", true);} $("#JSLoader").html(loader2); JsHttpRequest.query('modules/adm/fastlink-edit-JSReq.php',{'id':id, 'name':name, 'link':link},function(result,errors){if(result){ /*s*/ ActionAndUpdate(0,0); /*e*/ }},true); }
function AddNewCrossAct() { name=$('#Rn0').val(); link=$('#Rl0').val(); error=0; if (name=="" || name=="NULL"){ error=1; $("#Rn0").toggleClass("ErrorInput", true);} if (link=="" || link=="NULL"){ error=1; $("#Rl0").toggleClass("ErrorInput", true);} if (error==0) { $("#JSLoader").html(loader2); JsHttpRequest.query('modules/adm/fastlink-add-JSReq.php',{'name':name, 'link':link},function(result,errors){if(result){ /*s*/ ActionAndUpdate(0,0); /*e*/ }},true); }}
function ActionAndUpdate(id, act) { CloseBlank(); $("#Msg2").html("Идет сохранение данных..."); $("#Msg2").removeClass(); $("#Msg2").addClass("SaveDiv"); JsHttpRequest.query('modules/adm/fastlink-update-JSReq.php',{'id':id, 'act':act},function(result,errors){ if(result){  /*s*/ $("#Tgg").html(result["content"]); $("#Msg2").html("Данные успешно сохранены"); $("#Msg2").removeClass(); $("#Msg2").addClass("SuccessDiv"); /*e*/ }},true); }
