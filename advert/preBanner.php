...
$data=DB($q); if ($data["total"]==0) { echo "<h1>Error: Banner not found</h1>"; } else { @mysql_data_seek($data["result"], 0); $n=@mysql_fetch_array($data["result"]);
	...
	if ($n["html5"]){
		$html5 = '<iframe src="http://ng72.ru/advert/files/html5/' . $n['html5'] .'" width="'. $n['width'] .'" height="'. $n['height'] .'" style="border:0;"></iframe>';
		$html5 .= '<a href="'.$n['link'].'" target="_blank" style="position:absolute;top:0;left:0;display:block;height:100%;width:100%;"></a>';
		echo $html5;
	} else {
		if ($n["flash"]=="") {
