		...
		if (isset($F["advert"]["name"]) && $F["advert"]["name"]!="") {
			if($F["advert"]["ext"] != 'txt') {
				...
				foreach ($advertFile as $line){
					...
					if (preg_match('#ЧОрубрика#', $line)){
						if (AdvTagsClear($line)!="") {	
						$rubKey++;
						$rubSubKey = 0;
						$advRubs[$rubKey] = AdvTagsClear($line);
						}
					} else if (preg_match('#ЧОподрубрика#', $line)){
						$line = AdvTagsClear($line);
						if (!empty($line)){
							$rubSubKey++;
							$rubSubs[$rubKey][$rubSubKey] = $line;
						}
					} else if (preg_match('#ЧОтекст#', $line) || preg_match('#ЧОжирный#', $line) || preg_match('#ЧОквадрат#', $line)){
				...
				$handle = fopen($ROOT.'/template/allboard.html', 'w');	fwrite($handle, $html);	fclose($handle);
...
if($GLOBAL["USER"]["role"] > 2){
	...
	$text.="<div class='RoundText'>";
		$text.="<div title='Нажмите для выбора файла' class='BrowseButton'>Выбрать файл с компьютера<input type='file' id='advert' name='advert' onChange='getFileName();' /></div>";		
...
function AdvTagsClear($t){
	...
	//$t=preg_replace('/[^a-zA-Zа-яА-Я0-9_!\;\:\.\-\(\)]+/i', '', $t); 
	return trim($t);
}
?>