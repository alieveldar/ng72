...
if ($start=="view") {
	...
	if ($data["total"]) {
		...
		UserTracker($link, $page); $text.=UsersComments($link, $page, $item['comments']);
...
function GetConcursId($item) {
	...
	$text .= $item['text']; $path='/userfiles/picnews/'.$item["pic"];
	...
	if($GLOBAL["now"] > $item["votingend"]) {
		...
		$text.='<div class="C30"></div><h2 class="CenterText">Голосование окончено. '; $text.=$item['winnerscount'] == 1 ? 'Победитель:</h2>' : 'Победители:</h2>'; $text.='<div class="CBG"></div>';
	}
