...
if ($start=="view") {
	...
	$data=DB("SELECT `id`,`comments`, `promo`, `pic`,`name`,`title` FROM `".$table."` WHERE (".$alias." ".$where.") LIMIT 1");
	
	if ($data["total"]==1) {
		@mysql_data_seek($data["result"], 0); $new=@mysql_fetch_array($data["result"]); 
		$file=$table."-".$start."_mobile.".$new["id"].".".$id; $page=$new["id"];
		DB("UPDATE `".$table."` SET `seen`=`seen`+1 WHERE (`id`=".$new["id"].") LIMIT 1");
		
		if (RetCache($file)=="true") { list($text, $cap)=GetCache($file); } else { list($text, $cap)=GetLentaId(); SetCache($file, $text, $cap); }
		if ($Page404==0) { UserTracker($link, $page); $text.=$C20.UsersComments($link, $page, $new["comments"], 1, $page);  }
...
function GetLentaId() {
	...
	WHERE (`".$table."`.".$alias.") GROUP BY 1 LIMIT 1");
	...
	### Формирование данных
	$cap=$item["name"]; $Page["Crumbs"]="<div class='Crumbs'><a href='http://".$RealHost."'>Главная</a> &raquo; <a href='http://".$RealHost."/".$dir[0]."'>".$node["name"]."</a> &raquo; <a href='http://".$RealHost."/".$dir[0]."/cat/".$item["cat"]."'>".$item["ncat"]."</a> &raquo; ".$cap."</div>";
	...
	$dtags=''; $ar=explode(",", $item["tags"]); $q=""; if (count($ar)>2 && mb_strlen(strip_tags($maintext))>1000 && $item["promo"]!="1") { $limit=round(mb_strlen(strip_tags($maintext))/600); foreach ($ar as $k=>$v) { if ($v!='') { $q.="`tags` LIKE '%,".$v.",%' OR "; }}
	$data2=DB("SELECT `pic`,`data`,`name`,`id` FROM `".$table."` WHERE ((".trim($q, "OR ").") AND (`id`!='".(int)$dir[2]."') AND (`stat`='1')) ORDER BY `data` DESC LIMIT ".$limit); if ($data2["total"]>0) { $dtags='<div class="Dtags"><h3>Будь в теме:</h3>'; for ($i=0; $i<$data2["total"]; $i++): @mysql_data_seek($data2["result"],$i); $ar=@mysql_fetch_array($data2["result"]);
	$d=ToRusData($ar["data"]); $dtags.="<a href='/$dir[0]/view/$ar[id]/' title='".$ar["name"]."'>".$ar["name"]."</a><br><b>".$d[4]."</b><div class='C'></div><div class='CB'></div>"; endfor; $dtags.="<div class='C10'></div>Темы: ".$tags2.'</div>'; }}
	...
	} else { $text="<h1>".$cap."</h1><div class='WhiteBlock LentaItem' style='padding:9px;'>".$pic.$C15.$lid.$ban1."<div class='ItemText'>".$maintext.$ban.$dtags."</div>".$ban2.$report.$video.$album.$endtext.$voting.$event.$contacts.$C10.$likes.$mixblock.$ban3."</div>";}
...
function GetLentaCat() {
	...
	for ($i=0; $i<$data["total"]; $i++) { @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $d=ToRusData($ar["data"]); $pic="";
		...
		if (($i+1)%$onblock==0) { $text.="</div>".$C10."<div class='banner' id='Banner-6-".(floor($i/$onblock)+1)."'></div>".$C10."<div class='WhiteBlock'>"; } else { $text.=$C25; }
	}
