...
if ($start===0) {
	#$file="_tags-mcloud"; if (RetCache($file)=="true") { list($tags, $cap)=GetCache($file, 0); } else { list($tags, $cap)=TagsCloud(); SetCache($file, $tags, $cap); }	
	list($tags, $cap)=TagsCloud(); $cap="Теги публикаций"; $Page["Content"]=$tags; $Page["Caption"]=$cap;
}

### Вывод списка новостей общий
else {
	$data=DB("SELECT `name`,`name` FROM `".$table."` WHERE (`id`='".(int)$dir[1]."') LIMIT 1");
	if ($data["total"]==1){
		@mysql_data_seek($data["result"], 0); $tag=@mysql_fetch_array($data["result"]);
		#if (RetCache($file)=="true") { list($text, $cap, $tit)=GetCache($file); } else { list($text, $cap, $tit)=GetLentaList(); SetCache($file, $text, $cap, "", $tit); }
		list($text, $cap, $tit)=GetLentaList(); $Page["Content"]=$text; $Page["Caption"]=$tag["name"]; $Page["Title"]=$tag["title"];		
	}
	else {
		$cap="Тег не найден"; $text=@file_get_contents($ROOT."/template/404.html"); $Page["Content"]=$text; $Page["Caption"]=$cap;
	}	
}
...
function GetLentaList() {
	...
	$onpage=50; $pg = $dir[2] ? $dir[2] : 1;  $from=($pg - 1)*$onpage; $onblock=4;
	...
	$data=LSgetNewsFromLentas($q, $endq); $total=$data["total"]; $text.=Pager2($pg, $onpage, ceil($total/$onpage), $dir[0]."/".$dir[1]."/[page]"); return(array($text, $tag['name'], $tit));
}
