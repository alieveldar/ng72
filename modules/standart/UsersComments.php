...
function UsersComments($link, $pid, $sets, $header=1, $nodeid) {	
	...
	/*	$text.="<div id=\"unit_87776\"><a href=\"http://smi2.ru/\">Новости smi2.ru</a></div>
	...
	*/
	if ($sets==0 || $GLOBAL["USER"]["role"]>=1) {
		 if(($UserSetsSite[4]==1 && $GLOBAL["USER"]["id"]==0) || $GLOBAL["USER"]["id"]!=0) {
		 	if ($header==1) { $text.=$C5."<a name='addcomment' id='addcomment'></a><h2>Добавить комментарий</h2>".$C."<div class='UserCommentsForm'>".GetFormComments($link, $pid, $nodeid)."</div>"; } 
		 	if ($header==0) { $text.=$C5."<a name='addcomment' id='addcomment'></a>".$C."<div class='UserCommentsForm'>".GetFormComments($link, $pid, $nodeid)."</div>"; }
		 }
		 if(($UserSetsSite[4]==1 && $GLOBAL["USER"]["id"]==0) || $GLOBAL["USER"]["id"]!=0) {
		 	$text.="<script>jQuery.each($('.CommentAnswer'), function(i, val) { var fid=$(this).attr('id'); id=fid.split('-'); $(this).html(\"<a href='javascript:void(0);' onClick='CommentAnswer(\"+id[1]+\");'>ОТВЕТИТЬ</a>\"); });</script>";
		 } else {
...
function GetUsersComments($link, $pid) {
	...
	for ($i=0; $i<$data["total"]; $i++) {
		...
		$alt=$com["likes"]-$com["dislikes"]; if ($alt>$maxlikes && $alt>=10) { $maxlikesid=$com[id]; $maxlikes=$alt; }
	}
	...
	for ($i=0; $i<$data["total"]; $i++) { @mysql_data_seek($data["result"],$i); $com=@mysql_fetch_array($data["result"]); $datar=ToRusData($com["data"]); $medal=""; $toComment = ""; $cid=$com["id"];
		...
		if($com["toid"]){
			...
			$toComment .= "</div></div><div class='ToggleShow'><a href='javascript:void(0)' rel='nofollow' onclick='$(\".quote-overview\", $(this).parents(\".quote-container\")).removeClass(\"short\"); $(this).parents(\".ToggleShow\").hide()'>Развернуть</a></div></div></div>".$C5;
		}
		...
		$toComment = strtr($toComment, array("&#039;" => "'")); $toComment = strtr($toComment, $smiles); $comment = strtr($comment, array("&#039;" => "'")); $comment = strtr($comment, $smiles);
		// ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД ------------ ВЫВОД
		$text.="<a name='comment".$com["id"]."' id='comment".$com["id"]."'></a>"; $dopclass=""; $dophref1=""; $dophref2=""; if ($com["id"]==$maxlikesid) { $dopclass=" BestCom"; }
		if (($com["dislikes"]-$com["likes"])>=10) { $dopclass=" BedCom"; $dophref1="<div class='BadCommentA BadCommentA-".$com["id"]."'>Пользователи сочли этот комментарий бесполезным и мы его скрыли. <a href='javascript:void(0);' onclick='ShowBadComment(".$com["id"].");'>Показать</a></div><div class='ComHiddenPart-".$com["id"]."' style='display:none;'>"; $dophref2="</div>"; }
		$text.="<div class='CommentItem".$dopclass."' id='CommentItem-".$com["id"]."'>".$dophref1;
			$text.="<div class='LeftInfo'>".$avatar."<b>".$com["nick"]."</b>".$datar[1]."</div><div class='RightInfo'>".$answer."</div>".$C10.$toComment;
			$text.="<div class='view1'>".AntiMatFunc2($comment)."</div><div class='view2'></div>".$C10;
		
			#Вложения
			if (count($inc[$cid])>0) { $text.="<div class='CommentInc'>"; foreach($inc[$cid] as $k=>$pic) { $text.="<a href='/userfiles/comoriginal/".$pic."' rel='prettyPhoto[gallery]'><img src='/userfiles/compreview/".$pic."' /></a>";	} $text.=$C."</div>".$C10; }
			...
			$text.=$C5."<noindex>";
				...
				if (($com["dislikes"]-$com["likes"])>=10) { $text.="<div class='CommentBadLike' title='По версии читателей и количеству «-»'>Скрытый комментарий</div>";  }
...
// Форма комментариев
function GetFormComments($link, $pid, $nodeid) {
	...
	$text.="<div class='UserComTexts'>"; if ($GLOBAL["USER"]["id"]==0) { $text.="<div class='UserComNameDiv'><input class='UserComName' type='text' placeholder='Введите свое имя' value='".$_SESSION["username"]."' /><p class='or'>или авторизуйтесь</p>".GetUserAuthForm()."</div>".$C5; }
	...
	$text.=$C5."<div class='CommentSend'><input type='submit' name='sendbutton' class='SaveButton' value='Добавить комментарий' onClick=\"SendUserComment($(this).parents('.UserCommentsForm'), $nodeid);\"></div>";
