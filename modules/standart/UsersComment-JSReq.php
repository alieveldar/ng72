...
if($R["action"] == 'spam'){
	...
	if ($UserSetsSite[3]==1) {
		...
		if (strpos($_SESSION['userreferer'], "google.kz")===false) {} else { $error=1; $result=array("Code"=>0, "Text"=>"Ваш комментарий добавлен!", "Class"=>"SuccessDiv", "Comment"=>""); }
		if ($error==0) {
	...
	if ($error==0 && $UserSetsSite[3]==1) {
		...
		DB("INSERT INTO `_comments` (`link`,`pid`,`uid`,`uname`,`from`,`data`,`toid`,`text`,`ip`,`referer`) VALUES ('".$l."', '".$i."', '".$uid."', '".$n."', '".$ufrom."', '".time()."', '".$toid."', '".$t."', '".$GLOBAL["ip"]."', '".$_SESSION['userreferer']."');");
		$last=DBL();
...
function GL($l, $i, $t){ @file_put_contents($_SERVER['DOCUMENT_ROOT']."/modules/standart/debug-comment-add.txt", $l." / ".$i." / ".$t); }
