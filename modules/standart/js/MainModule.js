...
/* Инвайт в группу VK */
function showVKintive() {
    if (!$.cookie("vkinvite")) {
        $.cookie("vkinviter", "none", {expires: 1});
    }
    if ($.cookie("vkinviter") != "cancel" && pageclose == 0) {
        if (winmv == 0) {
            winmv = 1;
            VK.Widgets.Group("vk_group_invite", {
                mode: 0,
                width: "220",
                height: "275",
                color1: "FFFFFF",
        ...
        inmv = 1;
        $("#VKGroupInvite").fadeIn("normal");
        $("#closeVKinvate").click(function (e) {
            pageclose = 1;
            closeVKinvate();
        });
        $("#cancelVKinvate").click(function (e) {
            pageclose = 1;
            cancelVKinvate();
        });
    }
...
function closeVKinvate() {
    $("#VKGroupInvite").fadeOut("normal");
    inmv = 0;
}

function cancelVKinvate() {
    $("#VKGroupInvite").fadeOut("normal");
    $.cookie("vkinviter", "cancel", {expires: 100});
    inmv = 0;
}
...
function addLink() {
    ...
    var pagelink = "Источник: <a href='" + document.location.href + "'>" + document.location.href + "</a> © ng72.ru";
...
function timeryear() {
    ...
    timestr = "<div class='cif1'>" + today + "</div><div class='cif2'>" + thour + "</div>";
    $('#ngbg').html(timestr);
}
...
function SendUserComment(form, nodeid) {
    obj = $(".CommentMsg", form);
    obj.html("Идет отправка комментария...");
    obj.show();
    obj.attr("class", "SaveDiv");
    ...
    console.log("name=" + n + " | text=" + t + " | capcha=" + c + " | files=" + f + " | answer=" + an + " | nodeid=" + nodeid);
...
function likeSaveComment(qid, pid) {
    ...
    JsHttpRequest.query('/modules/standart/UsersComment-Like-JSReq.php', {
        ...
        if (result) { /*s*/
            $("#CommentLike-" + pid).html(result["text"]); /*e*/
        }
    }, true);
}
...
function BannersShow(pid, num, item) {
    ...
    if (($("#Banner-" + pid + "-" + num).size() != 0 && b[0] != 0) || pid == 4 || pid == 24) {
        ...
        if (view == 1) {
            var flash = '<object type="application/x-shockwave-flash" data="/advert/files/flash/' + b[4] + '" width="' + b[10] + '" height="' + b[11] + '"><param name="movie" value="/advert/files/flash/' + b[4] + '" /><param name="quality" value="high" /><param name="wmode" value="opaque" /><param name="flashvars" value="' + vars + '"><embed type="application/x-shockwave-flash"  width="' + b[10] + '" height="' + b[11] + '" quality="high" wmode="opaque" src="/advert/files/flash/' + b[4] + '" flashvars="' + vars + '"></embed></object>';
            obj.append(flash);
        }
        if (view == 2) {
        ...
        if (view == 6) {
            var html5 = '<div width="' + b[10] + '" height="' + b[11] + '" style="position: relative;"><iframe src="http://ng72.ru/advert/files/html5/' + b[14] + '" width="' + b[10] + '" height="' + b[11] + '" style="border:0;"></iframe><a href="' + link + '" target="_blank" style="position:absolute;top:0;left:0;display:block;height:100%;width:100%;"></a></div>';
            obj.append(html5);
            obj.show();
        }
        if (view == 7) {
            obj.append("<a href='" + link + "' target='_blank'><img src='/advert/files/image/" + b[5] + "' style='width:" + b[10] + "; height:" + b[11] + ";border:0; object-fit: cover;' />" + b[13] + "</a><div class='C'></div><div class='dater' style='float:left;'><a href='/ourspj'><b>" + b[15] + "</b></a></div>");
        }

        if (view != 0 && view != 4 && view != 5 && view != 6 && view != 7) {
            obj.css("width", b[10]);
            if (view != 13) {
                obj.css("height", b[11]);
            } else {
...
function BannersPodlogka(b) {
    if ($("#InnerCont").size() > 0) {
        ...
        $("#InnerCont").css("background", "url(/advert/files/image/" + b[5] + ") fixed center top no-repeat");
        if (b[12] != "") {
...
function ShowMoreInd() {
    ...
    JsHttpRequest.query('/modules/page_mods/ShowMoreInd-JSReq.php', {'lastdata': lastdata}, function (result, errors) {
        ...
        $("#ShowMoreInd").html("<a href='javascript:void(0);' onclick='ShowMoreInd();'>Показать больше новостей</a>");
