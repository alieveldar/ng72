...
if ($link == "news" && (int)$page == 9711) {
    header("Location: http://ng72.ru/bar.html");
    exit;
}

if ($link == "news" && (int)$page == 10203) {
    header("Location: http://ng72.ru/zukov.html");
    exit;
}
...
if ($start == "list") {
    if (RetCache($file) == "true") {
        list($text, $cap, $title, $kw, $ds) = GetCache($file);
    } else {
        list($text, $cap, $title, $kw, $ds) = GetLentaList();
        SetCache($file, $text, $cap, "cachepages", $title, $kw, $ds);
    }
}
...
if ($start == "date" && $link == "afisha") {
    if (RetCache($file) == "true") {
        ...
        list($text, $cap, $title, $kw, $ds) = GetAfishaList();
        SetCache($file, $text, $cap, "cachepages", $title, $kw, $ds);
    }
}
...
if ($start == "cat") {
    if (RetCache($file) == "true") {
        ...
        list($text, $cap, $title, $kw, $ds) = GetLentaCat();
        SetCache($file, $text, $cap, "cachepages", $title, $kw, $ds);
    }
}
...
if ($start == "view") {
    ...
    if (strpos($dir[2], "-") === false) {
        $alias = "`id`='" . (int)$dir[2] . "'";
    } else {
        $alias = "`alias`='" . $dir[2] . "'";
    }
    $data = DB("SELECT `id`,`alias`,`comments`, `promo`, `pic`,`name`,`title`,`seens`, `alttext` FROM `" . $table . "` WHERE (" . $alias . " " . $where . ") LIMIT 1");
    if ($data["total"] == 1) {
        @mysql_data_seek($data["result"], 0);
        $new = @mysql_fetch_array($data["result"]);

        if (strpos($dir[2], "-") === false && $new["alias"] != "") {
            @header("location: /" . $link . "/view/" . $new["alias"], true, 301);
            exit();
        }
        DB("UPDATE `" . $table . "` SET `seens`=`seens`+1 WHERE (`id`=" . $new["id"] . ") LIMIT 1");
        $file = $table . "-" . $start . "." . $new["id"] . "." . $id;
        $page = $new["id"];
        ...
        if($new["alttext"] == "") {
            if (RetCache($file) == "true") {
                ...
                list($text, $cap, $title, $kw, $ds) = GetLentaId();
                SetCache($file, $text, $cap, "cachepages", $title, $kw, $ds);
            }
        ...
        $text .= $C20 . '<div id="adfox_152941364840585683-v1"></div>
        <script>
        window.Ya.adfoxCode.create({
            ownerId: 272568,
            containerId: \'adfox_152941364840585683-v1\',
            params: {
                pp: \'g\',
                ps: \'cukh\',
                p2: \'fxrb\'
        ...
        $text .= UsersComments($link, $page, $new["comments"], null, $page);

        if ($new["promo"] != 1) { //$text.=$C20."<div class='banner' id='Banner-6-3'></div>";
        $text .= $C20 . '<div id="adfox_152941364840585683-v2"></div>
        <script>
        window.Ya.adfoxCode.create({
            ownerId: 272568,
            containerId: \'adfox_152941364840585683-v2\',
            params: {
                pp: \'g\',
                ps: \'cukh\',
                p2: \'fxrb\'
...
*/
        ...
        if ($new['seens'] > 499) {
            $kw = $cap . ", " . $VARS["keywords"];
            $ds = $cap . ", " . $VARS["description"];
            $cap .= "<span class='Act Seen' width='1%'>" . $new['seens'] . "</span>";
        }


        if ($GLOBAL["USER"]["role"] > 1) {
            $text = $C10 . "<div id='AdminEditItem'><a href='" . $GLOBAL["mdomain"] . "/admin/?cat=" . $link . "_edit&id=" . (int)$new["id"] . "'>Редактировать</a></div>" . $C15 . $text;
        }
    }else {
        $text           = $new["alttext"];
        $cap            = $new["name"];
...
function GetLentaList()
{
    ...
    for ($i = 0; $i < $data["total"]; $i++) {
        ...
        if ($UserSetsSite[3] == 1 && $ar["comments"] != 2 && (int)$ar["comcount"] != 0) {
        ...
        if ($data["total"] > ($i + 1)) {
            if (($i + 1) % $onblock == 0) {
                ...
                if (($i + 1) <= 2 * $onblock) {
                    $text .= $C25 . '
                    <div id="adfox_152941364840585683-' . ($i + 1) / $onblock . '"></div>
                    <script>
                    window.Ya.adfoxCode.create({
                        ownerId: 272568,
                        containerId: \'adfox_152941364840585683-' . ($i + 1) / $onblock . '\',
                        params: {
                            pp: \'g\',
                            ps: \'cukh\',
                            p2: \'fxrb\'
        ...
        for ($i = 0; $i < $data["total"]; $i++) {
            ...
            if ($data["total"] > ($i + 1)) {
                ...
                //$text .= $C25 . "<div class='banner2' id='Banner-6-" . ( floor( $i / $onblock ) + 1 ) . "'></div>" . $C;
                    if (($i + 1) <= 2 * $onblock) {
                        $text .= $C25 . '
                        <div id="adfox_152829433053614155-' . ($i + 1) / $onblock . '"></div>
                        <script>
                        window.Ya.adfoxCode.create({
                            ownerId: 272568,
                            containerId: \'adfox_152829433053614155-' . ($i + 1) / $onblock . '\',
                            params: {
                                pp: \'g\',
                                ps: \'cukh\',
                                p2: \'fxrb\'
            ...
            $ncat = $ar["ncat"];
            $data = DB("SELECT count(id) as `cnt` FROM `" . $table . "` WHERE (`cat`='" . (int)$dir[2] . "')");
            @mysql_data_seek($data["result"], 0);
            $ar = @mysql_fetch_array($data["result"]);
...
####### ВЫВОД СОДЕРЖАНИЯ НОВОСТИ ######################################################################################################################################
    ...
    ### Основной текст
            if ($item["data"] > 1464875848) {
                $maintext = "<p><b>Новости Тюмени и Тюменской области – " . date("d.m.Y", $item["data"]) . "</b></p>" . $item["text"];
            } else {
                $maintext = $item["text"];
            }
    ...
    ### Фото-отчет
            $p = DB("SELECT * FROM `" . $table2 . "` WHERE (`pid`='" . (int)$item["id"] . "' && `link`='" . $dir[0] . "' && `point`='report' && `stat`=1) order by `rate` ASC");
            $report = '';
            if ($p["total"] > 0) {
    ...
    ### Видео
                    $p = DB("SELECT * FROM `" . $table4 . "` WHERE (`pid`='" . (int)$item["id"] . "' && `link`='" . $dir[0] . "') LIMIT 1");
                    if ($p["total"] > 0) {
                        $video = "";
                        for ($i = 0; $i < $p["total"]; $i++): mysql_data_seek($p["result"], $i);
                            $ar = @mysql_fetch_array($p["result"]);
    ...
    ### Аватар автора, Автор и дата### Тэги
                            ...
                            if ($t != "") {
                                ...
                                for ($i = 0; $i < $ta["total"]; $i++): @mysql_data_seek($ta["result"], $i);
                                    $ar = @mysql_fetch_array($ta["result"]);
                                    $tags .= "<a href='/tags/$ar[id]'>$ar[name]</a>, "; endfor;
                                    $tags2 = trim($tags, ", ");
                                    $tags = "Тэги: " . trim($tags, ", ");
                                }
                                if ($item["avatar"] == "" || !is_file($ROOT . "/" . $item["avatar"]) || filesize($ROOT . "/" . $item["avatar"]) < 100) {
    ...
    $mixblock .= '<div id="adfox_152941364840585683-id1"></div>
    <script>
    window.Ya.adfoxCode.create({
        ownerId: 272568,
        containerId: \'adfox_152941364840585683-id2\',
        params: {
            pp: \'g\',
            ps: \'cukh\',
            p2: \'fxrb\'
    ...
    $mixblock .= '<div id="smi_teaser_11285">
    <center>
    <a href="https://24smi.info/?utm_source=informer_11285">
    ...
    if ($cdata["total"]) {
        ...
        $web = $con["web"];
        if ($con['name']) {
            $contacts = "<h2>" . $con["name"] . "</h2>";
        }
        if ($con["pic"]) {
            $contacts .= "<div style='float:left; margin-right:10px; width:120px;'><img src='/userfiles/picpreview/" . $con["pic"] . "' title='" . $con["name"] . "' width='120' /></div><div style='float:left; width:350px;'>";
        } else {
            $contacts .= "<div style='width:100%;'>";
        }
        if ($con["address"]) {
            $contacts .= "Адрес: " . $con["address"] . $C5;
        }
        if ($con["phone"]) {
            $contacts .= "Телефон: " . $con["phone"] . $C5;
        }
        if ($web) {
            $ri = str_replace(array("www.", "http://", "https://"), "", $web);
            $ri = trim($ri, "/");
            $ri = trim($ri);
            $ar1 = explode("/", $ri);
            $ar2 = explode("?", $ar1[0]);
            $web = "<a href='http://" . $ri . "' target='_blank' rel='nofollow'>" . $ar2[0] . "</a>";
            $contacts .= "Сайт: " . $web . $C5;
        }
        if ($con["anonce"]) {
            $contacts .= nl2br($con["anonce"]);
        }
        $contacts .= "</div>" . $C;
    }
    ...
    $realinfo = "";
    if ($item["realinfo"] != "") {
        if (strpos($item["realinfo"], "www.") !== false || strpos($item["realinfo"], "http") !== false || strpos($item["realinfo"], ".ru") !== false || strpos($item["realinfo"], ".com") !== false) {
            $ri = str_replace(array("www.", "http://", "https://"), "", $item["realinfo"]);
            $ri = trim($ri, "/");
            $ri = trim($ri);
            $ar1 = explode("/", $ri);
            $ar2 = explode("?", $ar1[0]);
    ...
    $nartext = '<div id="inpage_VI-288820-0-1106520692"></div>
    ...
    (function(w, d, n, s, t) {
        ...
        w[n].push(function() {
            Ya.Context.AdvManager.render({
                blockId: "VI-288820-0",
                renderTo: \'inpage_VI-288820-0-1106520692\',
                inpage: {
                    slide: true,
                    visibleAfterInit: false,
                    adFoxUrl: "//ads.adfox.ru/272568/getCode?pp=g&ps=cukh&p2=gbjm&fmt=1&pr="+pr,
...
// callback
                        ...
                        if ($item["data"] > 1464875848) {
                            $nartext .= "<noindex><p><b><a href='https://vk.com/ngt72' target='_blank' rel='nofollow'>Вступайте в НАШУ группу «ВКонтакте», узнавайте новости первыми! Там вас ждут: фото, видео, дискуссии, опросы</a></b></p></noindex>";
                        }
    ...
    $ddd .= '<div id="grf_gpp_ng72ru"></div>';


    ### компановка на вывод
    $text = $pic . $ban . "<div class='ArticleContent'>" . $lid . $maintext . "</div>" . $report . $video . $nartext . $endtext . $voting . AddVideo() . $album . $event . $contacts . $realinfo . $likes . $mixblock . $yandex . $readmore;
    // ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
    $cap = $item["name"];
    $title = $item["title"];
    $kw = $item["kw"];
    $ds = $item["ds"];
    return (array($text, $cap, $title, $kw, $ds, $ddd));
}
