...
if ($link=="news" && (int)$page==9711) {header("Location: http://ng72.ru/bar.html"); exit; }

if ($link=="news" && (int)$page==10203) {header("Location: http://ng72.ru/zukov.html"); exit; }
...
if ($start=="view") {
	...
	$data=DB("SELECT `id`,`alias`,`comments`, `promo`, `pic`,`name`,`title`,`seens` FROM `".$table."` WHERE (".$alias." ".$where.") LIMIT 1");
	if ($data["total"]==1) {
...
$mixblock .= '<div id="smi_teaser_10664"><center><a href="http://ex.24smi.info">новости</a></center></div>' . $C20;
	...
	if ($item["data"]>1464875848) {
		$nartext = '<!-- new block -->  <script type="text/javascript">
  ...
  <script>
      SeedrPlayer(document.getElementsByClassName("ArticleContent")[0], 50, {
        gid: "591c4eef5f366eee358b457f"
      });
  </script>';
		$nartext .= "<noindex><p><b><a href='https://vk.com/ngt72' target='_blank' rel='nofollow'>Вступайте в НАШУ группу «ВКонтакте», узнавайте новости первыми! Там вас ждут: фото, видео, дискуссии, опросы</a></b></p></noindex>"; } else { $nartext=""; }
