...
if ($start=="view") {
    ...
    $data = DB( "SELECT `id`,`alias`,`comments`, `promo`, `alttext`, `pic`,`name`,`title` FROM `" . $table . "` WHERE (" . $alias . " " . $where . ") LIMIT 1" );
    if ( $data["total"] == 1 ) {
        @mysql_data_seek( $data["result"], 0 );
        $new = @mysql_fetch_array( $data["result"] );

        DB( "UPDATE `" . $table . "` SET `seens`=`seens`+1 WHERE (`id`=" . $new["id"] . ") LIMIT 1" );
        $file = $table . "mobile-" . $start . "." . $new["id"] . "." . $id;
        $page = $new["id"];
        ...
        if($new["alttext"] == "") {
            ...
            $text .= UsersComments( $link, $page, $new["comments"], null,
                $page ) . '<div class="Banner" id="Banner-34-1"></div>' . $C15;;
            ...
            $Page["Title"] = $cap;
            $Page["Caption"] = "";
            $Page["MobileBottomContent"] = TheNewestInKazan(10);
...
function GetLentaId() {
    ...
    $ban1 = $C10 . '
    <div id="adfox_152941364840585683-1"></div>
    <script>
    window.Ya.adfoxCode.create({
        ownerId: 272568,
        containerId: \'adfox_152941364840585683-1\',
        params: {
            pp: \'g\',
            ps: \'cukh\',
            p2: \'fxrb\'
        ...
        $ban240x400 = "<!--Тип баннера: 240*400--><!--Расположение: <верх страницы>--><div style='width:240px;margin:0 auto;'><div id=\"adfox_153838416685239114\"></div></div><script>window.Ya.adfoxCode.create({ownerId: 272568,containerId: 'adfox_153838416685239114',params: {pp: 'g', ps: 'cukh',p2: 'fxqz'}});</script>";
        $ban200x333 = "<!--Тип баннера: 200*333--><!--Расположение: <верх страницы>--><div style='width:200px;margin:0 auto;'><div id=\"adfox_153838424982123670\"></div></div><script>window.Ya.adfoxCode.create({ownerId: 272568,containerId: 'adfox_153838424982123670',params: {pp: 'g', ps: 'cukh',p2: 'fxrc'}});</script>";
    ...
    ### Фото-отчет
            $p = DB( "SELECT * FROM `" . $table2 . "` WHERE (`pid`='" . (int) $dir[2] . "' && `link`='" . $dir[0] . "' && `point`='report' && `stat`=1) ORDER BY `rate` ASC" );
            $report = $C . "";
            if ( $p["total"] > 0 ) {
    ...
    ### Лайки
                        $likes = "<div class='Likes'>" . Likes( Hsc( $cap ), "", "http://" . $RealHost . $path,
                            Hsc( strip_tags( $lid ) ) ) . $C . "</div>" . $C10;
    ...
    ### Платные ссылки
                        ...
                        $mixblock .= $C10 . '
                        <div id="adfox_152941364840585683-2"></div>
                        <script>
                        window.Ya.adfoxCode.create({
                            ownerId: 272568,
                            containerId: \'adfox_152941364840585683-2\',
                            params: {
                                pp: \'g\',
                                ps: \'cukh\',
                                p2: \'fxrb\'
    ...
    ### Новости по таким же тэгам
                            ...
                            $dtags .= TheNewestInKazan( 4, $item["id"] ) . '<div class="Banner" id="Banner-33-1"></div>' . $C15;
...
// callback
    ...
    ### Текст вывода
                    ...
                    } else { $text="<h1>".$cap."</h1><div class='WhiteBlock LentaItem' style='padding:9px;'>".$pic.$C15.$ban240x400.$C15.$lid.$ban200x333.$C15.$ban1."<div class='ItemText'>".$maintext.$ban."</div>".$ban2.$report.$video.$album.$endtext.$voting.$event.$contacts.$C10.$nartext.$likes.$mixblock.$dtags.$ban3."</div>";}
