...
if ($start=="forums") {
	$VARS["cachepages"]=3; /* Задаем спец. кэш для главной форума  */
...
/* Вывод списка тем */
if ($start=="cat") 	{
	$VARS["cachepages"]=3; /* Задаем спец. кэш для тем форума  */
...
if ($start=="view") {
	$VARS["cachepages"]=3; /* Задаем спец. кэш для темы форума  */
	...
	if ($data["total"]==1) {
		...
		/* Комментарии */		if ($GLOBAL["USER"]["id"]!=0) { UserTracker($link, $page); } $text.=UsersComments($link, $page, $new["comments"], null, $page);
...
function GetForumForums() {
	...
	/* $q="SELECT
	`".$TABLES[1]."`.`id`, `".$TABLES[1]."`.`fid`, `".$TABLES[1]."`.`name`, `".$TABLES[1]."`.`update`, `".$TABLES[1]."`.`uid`, `".$TABLES[1]."`.`text`, `".$TABLES[1]."`.`tcnt`, `".$TABLES[1]."`.`comcount`,
	`_users`.`nick` 
	FROM `".$TABLES[1]."`
	LEFT JOIN `_users` ON `_users`.`id`=`".$TABLES[1]."`.`uid`
	WHERE (`".$TABLES[1]."`.`stat`=1)
	GROUP BY 1
	ORDER BY `".$TABLES[1]."`.`lock` DESC, `".$TABLES[1]."`.`rate` DESC"; */
	
	$q="SELECT `".$TABLES[1]."`.`id`, `".$TABLES[1]."`.`fid`, `".$TABLES[1]."`.`name`, `".$TABLES[1]."`.`update`, `".$TABLES[1]."`.`text`, `".$TABLES[1]."`.`tcnt`, `".$TABLES[1]."`.`comcount` 
	FROM `".$TABLES[1]."` WHERE (`".$TABLES[1]."`.`stat`=1) GROUP BY 1 ORDER BY `".$TABLES[1]."`.`lock` DESC, `".$TABLES[1]."`.`rate` DESC";
	$data=DB($q); for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); 
	$fid=$ar["fid"]; $cid=$ar["id"]; $items[$fid][$cid]=$ar; endfor;
	
	$q="select `cid`,`fid`,`id`,`name`,`update` from `".$TABLES[2]."` order by `update` ASC";
	$data=DB($q); for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $fid=$ar["fid"]; $cid=$ar["cid"]; 
	if ($ar["update"]>=$items[$fid][$cid]["max"]) { $items[$fid][$cid]["max"]=$ar["update"]; $items[$fid][$cid]["update"]=$ar["update"]; $items[$fid][$cid]["nick"]=$ar["name"]; $items[$fid][$cid]["uid"]=$ar["id"]; } endfor;
	...
	$data=DB("SELECT `id`,`name` FROM `".$TABLES[0]."` WHERE (`stat`=1) ORDER BY `rate` DESC"); for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); 
		...
		$text.="<tr class='TableHeader'><td width='48%'>Рубрики форума</td><td width='2%'> Темы </td><td width='2%'> Ответы </td><td width='48%'>Обновление</td></tr>".ForumChild($items[$ar["id"]], $ar["id"]);
		$text.="</table></div><div class='C10'></div>";
	endfor;

	/* $istime=time()-60*10; $text.="<h3>Сейчас на сайте:</h3>".$C5."<div class='WhiteBlock'>"; $data=DB("SELECT `id`,`nick` FROM `_users` WHERE (`lasttime`>'".$istime."') ORDER BY `lasttime` DESC LIMIT 100");
	for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]);
	$text.=" <a href='/users/view/".$ar["id"]."' style='font-size:10px; margin:1px 7px 1px 0;'><u>".$ar["nick"]."</u></a> "; endfor; $text.="</div>"; */
...
function GetForumCategory() {
	global $TABLES, $GLOBAL, $page, $ROOT, $node, $dir, $C10, $C, $C5, $link; $text=""; $onpage=$node["onpage"]; $pg = $dir[3]?$dir[3]:1; $from=($pg-1)*$onpage;
	...
	if ($data["total"]!=1) { $cap="Категория не найдена"; $text=@file_get_contents($ROOT."/template/404.html"); } else { @mysql_data_seek($data["result"], 0); $cat=@mysql_fetch_array($data["result"]);
		...
		if ($cat["add"]*$cat["fadd"]==1 || $GLOBAL["USER"]["role"]>1) {
			$add="<div class='AddForumTheme'><a href='/$dir[0]/addtheme/$cat[id]' rel='nofollow'>Создать новую тему</a></div><div class='ForumLink' style='float:right;'><a href='/$link/'>Вернуться в форум</a></div>"; } else { $add="<div class='ForumLink'><a href='/$link/'>Вернуться в форум</a></div>"; }
...
function AddForumTheme() {
	...
	if ((int)$GLOBAL["USER"]["id"]=="999") { $cap="Авторизация"; $text="Для создания новой темы необходимо авторизоваться".$C10.GetUserAuthForm(); } else {
	...
	if ($GLOBAL["USER"]["id"]==0) { $uname = trim(Dbcut(strip_tags($P['uname']))); if ($uname=="") { $uname="Горожанин"; } $_SESSION["username"]=$uname; } else { $uname=''; }
		...
		if ($newname=="") { $_SESSION["Msg"]='<div class="ErrorDiv">Внимание! Поле названия темы обязательно к заполнению. Тэги игнорируются!</div>'; } else {
			$q="INSERT INTO `".$TABLES[2]."` (`cid`, `fid`, `uid`, `uname`, `name`, `text`, `data`, `update`,`stat`) VALUES ('".(int)$cat["id"]."', '".(int)$cat["fid"]."', '".(int)$GLOBAL["USER"]["id"]."', '".$uname."', '".$newname."', '".$newtext."', '".time()."', '".time()."','1');";
			DB($q); $last=DBL();
...
###########################################################################################################################################################
function ForumUpdatedBy($uid, $nick, $update, $type=0) { global $link; $text="";  if($update=="") { return(""); }
	...
	$text="<a href='/".$link."/view/".$uid."'>".$nick."</a><br>".$data;
