...
function AllGallery() {
	...
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`pic`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,93,%' OR `[table]`.`tags` LIKE '%,72,%' OR `[table]`.`tags` LIKE '%,19,%'))";
	...
	$q="SELECT `[table]`.`id`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,93,%' OR `[table]`.`tags` LIKE '%,72,%' OR `[table]`.`tags` LIKE '%,19,%'))";
	$endq=""; $data=getNewsFromLentas($q, $endq); $text.=Pager2($pg, $onpage, ceil($data["total"]/$onpage), $ar["link"]."/".$dir[1]."/[page]"); return (array($text, $cap));
}
...
function PhotoGallery() {
	...
	$onpage=15; $pg=$dir[2]?$dir[2]:1; $from=($pg-1)*$onpage; $onblock=4;
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`pic`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,93,%'))";
	$endq="ORDER BY `data` DESC LIMIT $from, $onpage"; $data=getNewsFromLentas($q, $endq);
	for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $d=ToRusData($ar["data"]); $pic="";
		if ($ar["pic"]!="") { $pic="<img src='/userfiles/pictavto/".$ar["pic"]."' title='".$ar["name"]."' />"; }
		if ($UserSetsSite[3]==1 && $ar["comments"]!=2) { $coms="<div class='CommentBox'><a href='/".$ar["link"]."/view/".$ar["id"]."#comments'>".$ar["comcount"]."</a></div>"; } else { $coms=""; }
...
function VideoGallery() {
	...
	$onpage=15; $pg=$dir[2]?$dir[2]:1; $from=($pg-1)*$onpage; $onblock=4;
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`pic`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,72,%'))";
	$endq="ORDER BY `data` DESC LIMIT $from, $onpage"; $data=getNewsFromLentas($q, $endq);
	for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $d=ToRusData($ar["data"]); $pic="";
		if ($ar["pic"]!="") { $pic="<img src='/userfiles/pictavto/".$ar["pic"]."' title='".$ar["name"]."' />"; }
		if ($UserSetsSite[3]==1 && $ar["comments"]!=2) { $coms="<div class='CommentBox'><a href='/".$ar["link"]."/view/".$ar["id"]."#comments'>".$ar["comcount"]."</a></div>"; } else { $coms=""; }
...
function VideoGalleryTop() {
	...
	$text.="<div class='galleryHead' style='width:auto; height:460px' data-cols='5' data-rows='4' data-bigcell-limit='3' data-bg-size='cover' data-update-interval='10000' data-transition-time='500'>";
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`pic`, `[table]`.`data`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,72,%'))";
	$endq="ORDER BY `data` DESC LIMIT 30"; $data=getNewsFromLentas($q, $endq); for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); 
	$text.="<a  href='/$ar[link]/view/$ar[id]' title='$ar[name]'><img src='/userfiles/picitem/$ar[pic]' title='$ar[name]'></a>"; endfor; $text.="</div>".$C10; return (array($text, ""));
}
...
function MultimediaGallery() {
	...
	$onpage=15; $pg=$dir[2]?$dir[2]:1; $from=($pg-1)*$onpage; $onblock=4;
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`pic`, `[table]`.`data`, `[table]`.`comcount`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,19,%'))";
	$endq="ORDER BY `data` DESC LIMIT $from, $onpage"; $data=getNewsFromLentas($q, $endq);
	for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); $d=ToRusData($ar["data"]); $pic="";
		if ($ar["pic"]!="") { $pic="<img src='/userfiles/pictavto/".$ar["pic"]."' title='".$ar["name"]."' />"; }
		if ($UserSetsSite[3]==1 && $ar["comments"]!=2) { $coms="<div class='CommentBox'><a href='/".$ar["link"]."/view/".$ar["id"]."#comments'>".$ar["comcount"]."</a></div>"; } else { $coms=""; }
...
function MultimediaGalleryTop() {
	...
	$text.="<div class='galleryHead' style='width:auto; height:460px' data-cols='5' data-rows='4' data-bigcell-limit='3' data-bg-size='cover' data-update-interval='10000' data-transition-time='500'>";
	$q="SELECT `[table]`.`id`, `[table]`.`name`, `[table]`.`pic`, `[table]`.`data`, '[link]' as `link` FROM `[table]` WHERE (`[table]`.`stat`='1' && (`[table]`.`tags` LIKE '%,19,%'))";
	$endq="ORDER BY `data` DESC LIMIT 30"; $data=getNewsFromLentas($q, $endq); for ($i=0; $i<$data["total"]; $i++): @mysql_data_seek($data["result"], $i); $ar=@mysql_fetch_array($data["result"]); 
	$text.="<a  href='/$ar[link]/view/$ar[id]' title='$ar[name]'><img src='/userfiles/picitem/$ar[pic]' title='$ar[name]'></a>"; endfor; $text.="</div>".$C10; return (array($text, ""));
}
