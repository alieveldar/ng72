...
} // 0-рабочий режим, 1-режим отладки
...
$VarsToHtml = array(
    ...
    "Кнопка Про Город Объявления" => "SocialGroups",    // Социальные группы
    "Биржа ссылок" => "sape"
);
...
/* короткие ссылки */
$linkrep = preg_replace('/[^a-zA-Z0-9_\.\-]+/i', '', $link);
$qs = DB("SELECT `link` FROM `_fastlink` WHERE (`name`='" . $linkrep . "') LIMIT 1");
if ($qs["total"] == 1) {
    @mysql_data_seek($qs["result"], 0);
    $r = @mysql_fetch_array($qs["result"]);
...
if ($IsIndex == 1) {
    ...
    if (Dbsel($RealPage) != Dbsel($link)) {
        $q = "((`link`='" . Dbsel($RealPage) . "' && `module`='') || (`link`='" . Dbsel($link) . "' && `module`!=''))";
    } else {
        $q = "`link`='" . Dbsel($link) . "'";
    }
...
if ($data["total"] == 0) {
    ...
    if (trim($Domains[$node["domain"]] . "." . $VARS["mdomain"], ".") != $RealHost && $Perehvat != 2) {
        header("location: http://" . trim($Domains[$node["domain"]] . "." . $VARS["mdomain"], ".") . "/" . $RealPage);
        exit();
    }
    ...
    $Page["Node"] = $node;
    $Page["Link"] = $node["link"];
    if ($node['kw'] != "") $Page["KeyWords"] = $node["kw"];
    if ($node['ds'] != "") $Page["Description"] = $node["ds"];
    $Page["Data"] = $node["data"];
    ...
    if ($node["module"] == "") {
        ...
        if (is_file("modules/page_mods/" . $node["module"] . "-" . $node["link"] . ".php")) {
            ...
            /* JS */
            if (is_file("modules/page_mods/" . $node["module"] . "-" . $node["link"] . ".js")) {
                $JSmodules[$node["name"]] = "/modules/page_mods/" . $node["module"] . "-" . $node["link"] . ".js";
            }
            /* CSS */
            if (is_file("modules/page_mods/" . $node["module"] . "-" . $node["link"] . ".css")) {
            ...
            /* CSS */
            if (is_file("modules/" . $node["module"] . "/" . $node["module"] . ".css")) {
...
if ($node["link"] != "live" && $node["link"] != "index" && $node["link"] != "all" && ($node["module"] == "" || in_array($node["module"], $fortwocolumns))) {
    $design = "twocolumns";
}
...
if ($node["isindex"] == 1) {
    ...
    if ($Page["Title"] == "") {
        $Page["Title"] = strip_tags($Page["Caption"]) . " " . $VARS["splitter"] . " " . $VARS["sitename"];
    } else {
...
$RENDER .= '<script src="https://yastatic.net/pcode/adfox/loader.js" crossorigin="anonymous"></script>';
$RENDER .= '<script type="text/javascript" src="//code.giraff.io/data/widget-gpp_ng72ru.js" async></script>';
$RENDER .= '<script type="text/javascript" async src="https://vidtok.ru/js/videpotok_script_over.js"></script>';
...
$RENDER .= "<div id='VKGroupInvite'><div id='vk_group_invite'></div><div class='C10'></div><a href='javascript:void(0);' id='cancelVKinvate'>Закрыть окно</a><div class='C'></div></div>";
$RENDER .= '</body>' . $r . '</html>';
...
if ($SafeMode == 1 && $_SESSION["userid"] == 2) {
...
//}
// =============================================================================================================================================================================
//$logfile = fopen("logz.html" , "w");

//fwrite($logfile, $GLOBAL["log"]);
//fclose($logfile);
//var_dump($GLOBAL);
//die("SCRIPT OFF");
