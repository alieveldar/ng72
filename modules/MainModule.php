...
if ($SafeMode==1) { error_reporting(E_ALL & ~E_NOTICE); ini_set('display_errors', 'On'); } // 0-рабочий режим, 1-режим отладки
...
/* короткие ссылки */ $linkrep=preg_replace('/[^a-zA-Z0-9_\.\-]+/i', '', $link); $qs=DB("SELECT `link` FROM `_fastlink` WHERE (`name`='".$linkrep."') LIMIT 1"); if ($qs["total"]==1) { @mysql_data_seek($qs["result"], 0); $r=@mysql_fetch_array($qs["result"]); @header("location: ".$r["link"]); exit(); }

## ПЕРЕХВАТ СТАРЫХ URL ##############################################################
if ($link=="all") { $RealPage="all"; } if ($link=="oney") { $link="ls"; $Perehvat=2; $dir[0]="ls"; } if ($link=="gallery") { $RealPage="gallery"; }
...
$ICON='<link rel="shortcut icon" href="/favicon.png" type="image/x-icon" />'.$r; $fortwocolumns=array("forum", "strochki", "users", "eventmap", "2giscatalog", "companies");
if ($node["link"]!="live" && $node["link"]!="index" && $node["link"]!="all" && ($node["module"]=="" || in_array($node["module"], $fortwocolumns))) { $design="twocolumns"; }

if ($link=="gifts") { $design="valentine"; }


if ($node["design"]=="bar") { $design="bar"; }
...
if ($node["isindex"]==1) { $Page["Title"]=$VARS["sitename"]; } else { if ($Page["Title"]=="") { $Page["Title"]=strip_tags($Page["Caption"])." ".$VARS["splitter"]." ".$VARS["sitename"]; } else { $Page["Title"]=$Page["Title"]." ".$VARS["splitter"]." ".$VARS["sitename"]; }}
