...
$VarsToHtml = array(
	...
	"Тизер внизу, над копирайтом"					=> "MobileBottomContent" // формируется в модулях
...
/* короткие ссылки */ $linkrep=preg_replace('/[^a-zA-Z0-9_\.\-]+/i', '', $link); $qs=DB("SELECT `link` FROM `_crosslink` WHERE (`name`='".$linkrep."') LIMIT 1"); if ($qs["total"]==1) { @mysql_data_seek($qs["result"], 0); $r=@mysql_fetch_array($qs["result"]); @header("location: ".$r["link"]); exit(); }


if ($link=="index-mobile") { $RealPage="index-mobile"; }
